    ...
    return (
        <Text variant="text-md/normal" className={Margins.top16}>
            {matchingExcludedPlugins.length
                ? <>
                    <Forms.FormText>あなたはこれを見つけています:</Forms.FormText>
                    <ul>
                        {matchingExcludedPlugins.map(([name, reason]) => (
                            <li key={name}>
                                <b>{name}</b>: {ExcludedReasons[reason]} でのみ利用可能
                            </li>
                        ))}
                    </ul>
                </>
    ...
    return (
        <SettingsTab title="プラグイン">
            ...
            <div className={classes(Margins.bottom20, cl("filter-controls"))}>
                <TextInput autoFocus value={searchValue.value} placeholder="プラグインを検索..." onChange={onSearch} />
