# Vencord JP

[![Codebergのミラー (オリジナル)](https://img.shields.io/static/v1?style=for-the-badge&label=Codebergのミラー（オリジナル）&message=codeberg.org/Vee/cord&color=2185D0&logo=data:image/png;base64,)](https://codeberg.org/Vee/cord)

最もかわいい Discord のクライアント Mod

| ![image](https://github.com/Vendicated/Vencord/assets/45497981/706722b1-32de-4d99-bee9-93993b504334) |
| :--------------------------------------------------------------------------------------------------: |
|     [vencord-theme](https://github.com/synqat/vencord-theme)を使用して撮影したスクリーンショット     |

## 機能

-   とても簡単なインストール (インストーラーをダウンロード、開く、インストールボタンをクリック、完了)
-   100 を超える標準のプラグイン。リスト: https://vencord.dev/plugins
    -   ハイライト: SpotifyControls, MessageLogger, Experiments, GameActivityToggle, Translate, NoTrack, QuickReply, Free Emotes/Stickers, PermissionsViewer, CustomCommands, ShowHiddenChannels, PronounDB
-   多くのプラグインが標準で搭載されているのに、高速。
-   完璧なブラウザのサポート: 拡張機能またはユーザースクリプトを使用して Vencord をブラウザで実行。
-   すべての Discord ブランチで動作: 正式版、PTB、Canary、Development (最高の体験を得るには、正式版をご利用ください)
-   カスタム CSS とテーマ: あらゆる CSS ファイル（BetterDiscord テーマを含む）のインポートをサポートする内蔵 CSS エディタ。
-   プライバシーに配慮し、Discord のアナリティクスとクラッシュレポートをブロックし、テレメトリーもありません。
-   非常に活発にメンテナンスされており、壊れたプラグインは通常 12 時間以内に修正されます。
-   設定の同期： デバイス/アプリ間でプラグインとその設定を同期できます。（オプション）

## Vencord JP とは

Vencord JP は、日本語版の Vencord です。通常版の Vencord は英語です。

Discord は Discord Inc.の商標です。

Vencord JP は、Discord Inc. 及び Vencord と連携または推奨しているものではありません。

## インストールとアンインストール

### Windows

**インストール**

1. Windows PowerShell を開きます。(Win+R->powershell->Enter)
2. 下記のコマンドをペーストします。
    ```powershell
    iex ((new-object net.webclient).DownloadString('https://go.uplauncher.xyz/vencord-jp.ps1'))
    ```
3. (オプション) クライアントが複数ある場合、クライアントを選択します。
4. 「Install」ボタンをクリックして、Vencord JP をインストールします。
5. (オプション) 「Install OpenAsar」をクリックして OpenAsar をインストールします。
   **アンインストール**
6. Windows PowerShell を開きます。(Win+R->powershell->Enter)
7. 下記のコマンドをペーストします。
    ```powershell
    iex ((new-object net.webclient).DownloadString('https://go.uplauncher.xyz/vencord-jp.ps1'))
    ```
8. (オプション) クライアントが複数ある場合、クライアントを選択します。
9. 「Uninstall」ボタンをクリックして、Vencord JP をアンインストールします。
10. (オプション) 「Uninstall OpenAsar」をクリックして OpenAsar をアンインストールします。

### Mac / Linux / Browser

現在、準備中です。もうしばらくお待ちください。

## サポート

翻訳以外に関するサポートは、[Vencord の Discord サーバー](https://discord.gg/D9uwnFnqmd)でサポートを受けてください。(英語)

翻訳に関するサポートは、[雷の Discord サーバー](https://go.uplauncher.xyz/discord)でサポートを受けてください。

OpenAsar に関するサポートは、[OpenAsar Issues](https://github.com/GooseMod/OpenAsar/issues)でサポートを受けてください。

## スポンサー（オリジナル）

| **すべての Vencord（オリジナル）のスポンサー [オリジナルの Vencord をサポート](https://github.com/sponsors/Vendicated)!!** |
| :------------------------------------------------------------------------------------------------------------------------: |
|                  [![](https://meow.vendicated.dev/sponsors.png)](https://github.com/sponsors/Vendicated)                   |
|            _[github-sponsor-graph](https://github.com/Vendicated/github-sponsor-graph)を使用して作成しました。_            |

## スターの履歴（オリジナル）

<a href="https://star-history.com/#Vendicated/Vencord&Timeline">
  <picture>
    <source media="(prefers-color-scheme: dark)" srcset="https://api.star-history.com/svg?repos=Vendicated/Vencord&type=Timeline&theme=dark" />
    <source media="(prefers-color-scheme: light)" srcset="https://api.star-history.com/svg?repos=Vendicated/Vencord&type=Timeline" />
    <img alt="Star History Chart" src="https://api.star-history.com/svg?repos=Vendicated/Vencord&type=Timeline" />
  </picture>
</a>

## 注意点

Discord is trademark of Discord Inc. and solely mentioned for the sake of descriptivity.
Mention of it does not imply any affiliation with or endorsement by Discord Inc.

<details>
<summary>Using Vencord violates Discord's terms of service</summary>

Client modifications are against Discord’s Terms of Service.

However, Discord is pretty indifferent about them and there are no known cases of users getting banned for using client mods! So you should generally be fine as long as you don’t use any plugins that implement abusive behaviour. But no worries, all inbuilt plugins are safe to use!

Regardless, if your account is very important to you and it getting disabled would be a disaster for you, you should probably not use any client mods (not exclusive to Vencord), just to be safe

Additionally, make sure not to post screenshots with Vencord in a server where you might get banned for it

</details>
